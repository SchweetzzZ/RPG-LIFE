...
export class CharacterClassService {
    ...
    async createCharacterClass(data: CreateCharacterClassDto) {
    ...
    async updateCharacterClass(data: UpdateCharacterClassDto) {
        const exists = await this.characterClassModel.findOne({ name: data.name });
        if (!exists) {
            throw new ConflictException(`A classe "${data.name}" não existe!`);
        }

        return this.characterClassModel.updateOne({ name: data.name }, data);
    }

    async deleteCharacterClass(data: UpdateCharacterClassDto) {
        const exists = await this.characterClassModel.findOne({ name: data.name });
        if (!exists) {
            throw new ConflictException(`A classe "${data.name}" não existe!`);
        }

        return this.characterClassModel.deleteOne({ name: data.name });
    }
